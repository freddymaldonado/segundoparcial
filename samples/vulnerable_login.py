...
# Simula un modulo de login "vibe-coded" con multiples fallos de seguridad.

import hashlib
import sqlite3

# FALLO: secreto hardcodeado en el codigo
API_KEY = "sk-prod-9f8e7d6c5b4a3210ffeeddccbbaa9988"
DB_PATH = "users.db"


def hash_password(password):
    # FALLO: MD5 es un hash debil para passwords
...
def login(username, password):
    ...
    cursor = conn.cursor()
    # FALLO: SQL injection por f-string, sin parametrizar
...
def reset_password(username, new_password):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    # FALLO: misma inyeccion, ademas sin verificar identidad del solicitante
    cursor.execute(
